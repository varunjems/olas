...
<div id="idform" class="form right">
    <form class="frm-setup frm-signin" method="post" action="#a">
        ...
        <div class="control-group">
            ...
            <label for="student_name_first_name">First Name: </label>
            <?php echo $form['first_name']->render() ?>
            <?php // <input type="text" name="student_name[first_name]" id="student_name_first_name" maxlength="28" required="required" /> ?>
        </div>
        ...
        <div class="control-group">
            ...
            <?php endif; ?>
            <label for="student_name_last_name">Last Name: </label>
            <?php echo $form['last_name']->render() ?>
            <?php //<input type="text" name="student_name[last_name]" id="student_name_last_name" maxlength="28" required="required" /> ?>
        </div>
    </form>
</div>
...
<script>
    ...
    $(document).on('change', 'form', function(e) {
        ...
        $('#student_name_first_name').removeClass('error');
        if ($('#student_name_first_name').val().length == 0) {
            // Need to warn user (if never warned before)
            $('.btn-next').addClass('emptyField');
        } else if ($('#student_name_first_name').val().length > 28) {
            enable = 0;
            $('#student_name_first_name').addClass('error');
        } else if (!removeDiacritics($('#student_name_first_name').val()).match(/^[a-zA-Z\.-][a-zA-Z\. -]*[a-zA-Z\.-]$/)) {
            enable = 0;
            $('#student_name_first_name').addClass('error');
        }

        $('#student_name_last_name').removeClass('error');
        if ($('#student_name_last_name').val().length == 0) {
            // Need to warn user (if never warned before)
            $('.btn-next').addClass('emptyField');
        } else if ($('#student_name_last_name').val().length > 28) {
            enable = 0;
            $('#student_name_last_name').addClass('error');
        } else if (!removeDiacritics($('#student_name_last_name').val()).match(/^[a-zA-Z\.-][a-zA-Z\. -]*[a-zA-Z\.-]$/)) {
            enable = 0;
            $('#student_name_last_name').addClass('error');
        }
  ...
  $(document).on('click', '.btn-back', function(e) {
      ...
      $('.btn-back').animate({left:'-=100'}, 300, function(){
          window.location = '<?php echo url_for('elem_student_signin_sid', array('assessment'=>$assessment, 'school'=>$school, 'sid'=>$sid)) ?>#a';
      });
  });
  ...
  $("#player1").jPlayer({
   ready: function () {
    $(this).jPlayer("setMedia", {
      mp3: "<?php echo image_path('/audio/others/student_name.mp3') ?>",
      oga: "<?php echo image_path('/audio/others/student_name.ogg') ?>"
    });
   },
